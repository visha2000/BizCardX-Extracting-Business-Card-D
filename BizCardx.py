...
con=pymysql.connect(
    ...
    port = 3306 )
mycursor=con.cursor()
mycursor.execute("Create database if not exists project3")
mycursor.execute("use project3")

st.markdown("<h1 style='text-align: center; color: violet;font-size: 30px;'>BizCardX: Extracting Business Card Data with OCR</h1>",
            unsafe_allow_html=True)
#select option
GETOPTIONS = st.sidebar.selectbox('[Busines Card Data Extraction]',["Home", "Upload&Modify", "Delete"])

# HOME MENU
if GETOPTIONS == "Home":   
    st.write("""### :green[**OCR :**] Optical Character Recognition (OCR) is the process that converts an image of text into a machine-readable text format.""")
    st.write("""### :green[**Importance Of OCR :**] OCR technology solves the problem by converting text images into text data that can be analyzed by other business software.You can then use the data to conduct analytics, streamline operations, automate processes, and improve productivity.""")
    st.write("### :green[**Technologies Used in this Project :**] OCR,Streamlit GUI,SQL,Data Extraction")
...
if GETOPTIONS == "Delete":
        ...
        if name_selected  and remove:
            mycursor.execute(f"DELETE FROM BUSINESS_CARD WHERE NAME = '{name_selected}'")
            con.commit()
            if remove:
